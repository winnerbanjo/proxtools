  ...
  return (
    <DashboardShell userName={user.name}>
      ...
      <Card className="mt-4">
        ...
        <DataTable headers={["S/N", "Type", "Country / Network", "Amount", "Status", "Created At"]} rows={data.orders.map((order, index) => [index + 1, order.kind, order.country || order.network || "-", money(order.amount), <Badge key={order.id}>{order.status}</Badge>, shortDate(order.createdAt)])} empty="No orders yet." />
      </Card>
    </DashboardShell>
  );
}
